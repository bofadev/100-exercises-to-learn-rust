...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(error) => {
            match error {
                TicketNewError::TitleError { details } => {
                    panic!("{details}")
                },
                TicketNewError::DescriptionError { details } => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { details: "Title cannot be empty".to_string() })
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { details: "Title cannot be longer than 50 bytes".to_string() })
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { details: "Description cannot be empty".to_string() })
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { details: "Description cannot be longer than 500 bytes".to_string() })
        }
